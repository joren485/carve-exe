        ...
        while b := h_input.read(1):
            ...
            for filetype in FILETYPES:
                ...
                if b == filetype.MAGIC:
                    ...
                    logger.debug(f"@0x{executable_offset:x}: {filetype.NAME} candidate")

                    # handle
                    executable_size = filetype.get_size(h_input)
                    if executable_size is not None and executable_size < 2**16:
                        ...
                        if len(executable_data) < executable_size:
                            ...
                            logger.success(f"Wrote {filetype.NAME} of {len(executable_data)} bytes to {path_out}")

                        else:
                            logger.debug(f"@0x{executable_offset:x} Failed to read {executable_size} bytes")

                    else:
                        logger.debug(f"@0x{executable_offset:x} Failed to load {filetype.NAME} candidate")
